		...
		case SRV_MAC_LINK_ADR_REQ:
			{
				...
				else
				{
					for ( i = 0; i < LORA_MAX_NB_CHANNELS; i++ )
					{
						...
						else if ( ( chMask & ( 1 << i ) ) != 0 && Channels[i].Frequency == 0 )
							status &= 0xFE; // Channel mask KO
					}
				}
...
static void OnRadioRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr )
{
	...
	if ( LoRaMacDeviceClass != CLASS_C )
		Radio.Sleep( );
	else if ( LoRaMacEventFlags.Bits.RxSlot == 0 )
		OnRxWindow2TimerEvent( );
...
static void OnMacStateCheckTimerEvent( void )
{
	...
	else	// Operation not finished restart timer
		TimerStart( &MacStateCheckTimer );
}
...
void LoRaMacSetPublicNetwork( bool enable )
{
	...
	if ( PublicNetwork == true )	// Change LoRa modem SyncWord
		Radio.Write( REG_LR_SYNCWORD, LORA_MAC_PUBLIC_SYNCWORD );
	else	// Change LoRa modem SyncWord
